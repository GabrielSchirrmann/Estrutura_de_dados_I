/*
 * Aula 06 - Grafos
 * 5. Grafo PONDERADO, DIRECIONAL - Matriz de Adjacencias
 */
#include <stdio.h>
#include <stdlib.h>
...
int criarAresta(GRAFO *gr, int vi, int vf, int peso) {
    ...
    gr->adj[vi][vf] = peso; /* direcional: apenas vi -> vf */
    gr->aresta++;
    return 1;
}
...
int main(void) {
    /* Exemplo dos slides: V1->V2=2, V1->V3=10, V2->V3=5, V4->V2=8 */
    GRAFO *gr = criarGrafo(4);
    criarAresta(gr, 0, 1, 2);
    criarAresta(gr, 0, 2, 10);
    criarAresta(gr, 1, 2, 5);
    criarAresta(gr, 3, 1, 8);

    imprimir(gr);
    printf("Peso (0->2): %d\n", pesoAresta(gr, 0, 2));
    printf("Grau saida v1: %d\n", grauSaida(gr, 1));
